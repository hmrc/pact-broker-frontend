...
class PactJsonFilesExecutor @Inject()(
  ...
  def execute():Future[PactJsonFilesExecutorResult] = {
    ...
    val (errors, pactsWithVersion) = pactFileLoadResults.foldLeft[ResultLists](List[String]() -> List[PactWithVersion]()) { (resultLists:ResultLists, result:Either[String, PactWithVersion]) =>
      result match {
    ...
    Future.sequence(pactsWithVersion.map { pactWithVersion =>
      pactService.addPactTest(pactWithVersion.provider.name, pactWithVersion.consumer.name, pactWithVersion).map{
        case result@Right(_) => logger.info(s"[GG-5850] Pact added.. provider: ${pactWithVersion.provider.name}, consumer: ${pactWithVersion.consumer.name}, version: ${pactWithVersion.version}"); result
        case result@Left(error) => logger.error(s"[GG-5850] Failed to add Pact.. provider: ${pactWithVersion.provider.name}, consumer: ${pactWithVersion.consumer.name}, version: ${pactWithVersion.version}, error:${error}"); result
      }
