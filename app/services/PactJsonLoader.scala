...
class PactJsonLoader @Inject()(config: PactBrokerConfig) extends Logging {
  ...
  private def getResources(pattern: String): Seq[PactJsonLocation] = {
    ...
    val classPathResourcesNotFoundAlready = classPathResources.filter(_.location.split("/").reverse.headOption.fold(false)(!resourceNamesInJar.contains(_)))
  ...
  private def findResourcesInJarFile(jarFile: File, pattern: Pattern):Seq[ResourcePath] = {
    Try {
      ...
      case Success((found, total)) if found.nonEmpty => logger.info(s"[GG-5850] ${found.size}/${total.size} entries matched in ${jarFile.getName}."); found.map(ResourcePath(_))
  ...
  private def findFilesInDirectory(directory: File, pattern: Pattern):Seq[FilePath] = {
    ...
    val files = fileList.filter { file =>
